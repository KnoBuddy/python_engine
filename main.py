...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            rpm_rect = ui.draw_slider(screen, (width // 2 - 200, height - 150), 400, 10, 10, rpm, 0, 5000, 'RPM', rpm_input_value, rpm_input_active)
            speed_rect = ui.draw_slider(screen, (width // 2 - 200, height - 80), 400, 10, 10, speed_factor, 0.01, 1.0, 'Speed', speed_input_value, speed_input_active)
            rpm_input_active, speed_input_active, rpm_input_value, speed_input_value = ui.handle_mouse_click(
                event, rpm_rect, speed_rect, rpm_input_value, speed_input_value, rpm_input_active, speed_input_active
            )
    ...
    rpm, speed_factor = ui.handle_slider_movement(event, pygame.Rect(width // 2 - 200, height - 150, 400, 10), pygame.Rect(width // 2 - 200, height - 80, 400, 10), rpm, speed_factor)
